...
- Evaluasi dilakukan per query dan per model/source pool agar recall tidak tercampur antar model
...
def run_experiment() -> None:
    ...
    # Penting:
    # File manual labeling berisi beberapa pool/model untuk query yang sama.
    # Metrik harus dihitung dalam pool model masing-masing, bukan digabung per query.
    ...
    for (pop_id, pop_features), (val_id, val_features) in feature_pairs:
        ...
        for scenario, w in WEIGHT_SCENARIOS.items():
            ...
            # Evaluasi per query + source_model.
            # Contoh: query "kopi khas daerah" pada model bm25 dievaluasi hanya terhadap 70 kandidat bm25,
            # bukan terhadap 350 kandidat gabungan dari bm25 + balance + relevance + popularity + value.
            ...
            # Ringkasan dibuat per source_model agar hasil tiap model tidak tercampur.
