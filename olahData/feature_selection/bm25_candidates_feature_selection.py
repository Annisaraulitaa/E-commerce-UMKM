...
└── bm25_candidates_all_queries.csv
...
Cara menjalankan:
python bm25_candidates_feature_selection.py --input "D:/Kuliah/DATA_TA/olahData/nondup_labeled_dataset(new).csv"
"""
...
class BM25CandidateEngine:
    ...
    def get_candidates(self, query: str, top_n: int = TOP_N_CANDIDATES) -> pd.DataFrame:
        ...
        df_out["query"] = query
        df_out["rank_bm25"] = range(1, len(df_out) + 1)
    ...
    def run_many_queries(
        ...
        output_dir: str = "output_bm25_candidates",
    ) -> pd.DataFrame:
        ...
        for i, query in enumerate(queries, start=1):
            ...
            candidates = self.get_candidates(query=query, top_n=top_n)

            print(f"  - candidates Top-{top_n}: {len(candidates)} baris")
        ...
        output_file = output_path / "bm25_candidates_all_queries.csv"
        candidates_all.to_csv(output_file, index=False, encoding="utf-8-sig")

        print("\nSelesai.")
        print(f"Candidates disimpan: {output_file}")
        print(f"Total baris output : {len(candidates_all):,}")
