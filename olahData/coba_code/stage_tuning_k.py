...
def evaluate_for_k(k):
    ...
    for q in TEST_QUERIES:
        ...
        rows.append({
            ...
            "F1": f1_at_k(
                precision_at_k(bm25_res, q, k),
                recall_at_k(bm25_res, candidate_pool, q, k)
            ),
        ...
        rows.append({
            ...
            "F1": f1_at_k(
                precision_at_k(hybrid_res, q, k),
                recall_at_k(hybrid_res, candidate_pool, q, k)
            ),
...
def build_stage1_outputs(df_results):
    ...
    summary = (
        df_results
        .groupby(["method", "K"])[["Precision", "Recall", "F1", "NDCG", "Fairness"]]
        .mean()
        .reset_index()
    )
